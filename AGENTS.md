...
- **Card colors**: driven by `data-type="word"` / `data-type="def"` attribute on `.memory-card`; CSS attribute selectors `[data-type="word"] .memory-card-front` (gold gradient) / `[data-type="def"] .memory-card-front` (blue gradient); same for `.memory-card-back` (parchment gold vs blue-grey)
