...
export async function genTranslation(){
  ...
  try{
    const raw=await callLLM(`Eres ${chars[R.cur].name}, generando ejercicios de traducción inglés→español para un estudiante de nivel ${LEVELS[S.level]}.`,[{role:'user',content:`${GAME_DIFF[S.gameDifficulty].prompt} Tema: ${topic}. Genera UNA frase corta en INGLÉS sobre ese tema para que el estudiante la traduzca al español.${avoid} Refleja tu personalidad. RESPONDE SOLO con este JSON: {"phrase":"frase en inglés","refTranslation":"una traducción correcta al español"}`}],150);
...
Responde SOLO con: {"status":"correct|minor|incorrect","correction":"traducción correcta","note":"breve explicación en español"}`}],150,{temperature:0.2});
